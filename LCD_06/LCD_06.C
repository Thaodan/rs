...
char daten[8]={0x04, 0x0E, 0x1F, 0x04, 0x04, 0x04, 0x04, 0x1F};	
//-------------------------------- P r o t o t y p e n ----------------------------------
void main(void);
...
char getchar(void);
void printhex(unsigned char wert);	// 00 bis FF seriell ausgeben
void progcharline(unsigned char adresse, unsigned char daten[8]);
unsigned char *readdata(void);
//------------------------------ H a u p t p r o g r a m m ------------------------------
void main(void) 
  {
  unsigned char c, *cp=daten;
  ...
  for(;;)
    ...
    putchar(c);
    putchar('\n');
    progcharline(c-'0', daten); // programmieren (fuktioniert nicht mit cp statt daten? )

    gotozs(0,0);
    putlcd(c-'0');  // Ausgabe des programmierten Zeichens
    gotozs(1,0);
    putlcd(c-'0');
    gotozs(2,0);
    putlcd(c-'0');
  ...
  printstring("\nAbbruch mit escape\n");
                                 ...
                                 // <<<< HIER Nutzung der programmierten Zeichen
...
void init_lcd(void)
  ...
  *WRITE_INS=0x34; // *WRITE_INS=0x34;	// Function Set: 8 Bit, RE=1
  wait(1);		// erforderlich 39µs
  *WRITE_INS=0x09; // *WRITE_INS=0x09;	// extended Function Set: 4 Zeilen Modus
  wait(1);		// erforderlich 39µs
